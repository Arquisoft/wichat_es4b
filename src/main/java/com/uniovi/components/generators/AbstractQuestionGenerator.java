...
public abstract class AbstractQuestionGenerator<T extends AbstractQuestion>
	...
	protected AbstractQuestionGenerator(JsonNode jsonNode) {
	...
	@Override
	public List<T> getQuestions(String language)
	...
	@Override
	public List<T> getQuestions(String language, JsonNode question, Category cat)
			throws IOException, InterruptedException {
		this.language = language;
		return this.generateQuestion(question, cat);
	}

	protected abstract List<T> generateQuestion(JsonNode question, Category cat)
			throws IOException, InterruptedException;

	// Método para obtener las opciones
	protected List<Answer> generateOptions(JsonNode results, String correctAnswer,
										   String answerLabel) {
		...
		while (options.size() < 3 && tries < 10) {
			...
			String option = results.get(randomIdx).path(answerLabel).path("value")
					.asText();
	...
	protected abstract JsonNode getQueryResult(String query)
			throws IOException, InterruptedException;
}
