...
public class QuestionImageGeneratorImpl extends AbstractQuestionGenerator<QuestionImage> {
	...
	private static final Logger logger = LoggerFactory.getLogger(QuestionImageGeneratorImpl.class);
	...
	@Override
	protected List<QuestionImage> generateQuestion(JsonNode question, Category cat) {
		...
		do {
			try {
				...
				logger.error("Ocurrió un error al obtener las preguntas: {}", e.getMessage(), e);
				// TODO pendiente de definir si se expande el error en caso de necesidad.
			}
...
// Verificamos que haya resultados antes de procesarlos
		if (results != null && !results.isEmpty()) {
			...
			for (String[] result : results) {
				...
				List<AnswerImage> options = this.generateOptionsL(results, correctAnswer);
	...
	protected List<AnswerImage> generateOptionsL(List<String[]> results, String correctAnswer) {
	...
	@Override
	protected JsonNode getQueryResult(String query) {
		return null;
	}
	...
	public static List<String[]> getQueryResultL() {
		...
		try {
			...
			logger.error("Ocurrió un error al ejecutar la consulta SPARQL: {}", e.getMessage(), e);
		}
