...
public class ImageQuestionGenerator {
    ...
    private Logger logger = LoggerFactory.getLogger(ImageQuestionGenerator.class);

    public ImageQuestionGenerator(JsonNode jsonNode) {
    ...
    public List<ImageQuestion> getImageQuestions(String language) throws IOException, InterruptedException {
        ...
        for(JsonNode category : categories){
            ...
            for(JsonNode question : questionsNode){
                questions.addAll(this.generateImageQuestions(question, cat));
            }
        }
        return questions;
    }
    ...
    public List<ImageQuestion> getImageQuestions(String language, JsonNode question, Category cat) throws IOException, InterruptedException {
        this.language = language;
        return this.generateImageQuestions(question, cat);
    }

    private List<ImageQuestion> generateImageQuestions(JsonNode question, Category cat) throws IOException, InterruptedException {
        ...
        for (JsonNode result : results) {
            ...
            if (statement != null) {
                ...
                String imageUri = result.path(imageLabel).path("value").asText();

                // Generate the question
                ImageQuestion q = new ImageQuestion(questionStatement, options, cat, language, imageUri);
