...
public class PlayerApiController {
	...
	public static final String HTTP_URL = "/api/players";
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Success",
					content = {@Content(mediaType = RESP_TYPE, examples =
							{@ExampleObject(name = DESC_EXP, value = "{ \"success\" : true, \"id\": 1 }")}
					)}),
			@ApiResponse(responseCode = "401", description = DESC_INVALID_KEY,
					content = @Content(mediaType = RESP_TYPE, examples =
							{@ExampleObject(name = DESC_ERR, value = RESP_INVALID_API_KEY)}
					)),
			@ApiResponse(responseCode = "400", description = "Could not add user due to validation errors",
					content = @Content(mediaType = RESP_TYPE, examples = {@ExampleObject(name = DESC_ERR,
							value = "{\"field1\":\"Error description in field 1\", \"field2\":\"Error description in field 2\"}"
					)}))
	})
	public String createPlayer(@RequestHeader(name = API_KEY_HEADER) String apiKeyStr, @RequestBody PlayerDto playerDto,
		...
		restApiService.logAccess(apiKey, HTTP_URL, Map.of("apiKey", apiKey.getKeyToken(), "http-method", "post", "user", playerDto.toString()));
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Success",
					content = {@Content(mediaType = RESP_TYPE,
							examples = {@ExampleObject(name = DESC_EXP,
									value = """
											{
											   "players":[
											      {
			...
			@ApiResponse(responseCode = "401", description = DESC_INVALID_KEY,
					content = @Content(mediaType = RESP_TYPE, examples = {@ExampleObject(name = DESC_ERR,
							value = RESP_INVALID_API_KEY
					)}))
	})
	public String listPlayers(@RequestParam @Parameter(hidden = true) Map<String, String> params,
							  HttpServletResponse response) throws JsonProcessingException {
		// Tipo de respuesta
		response.setContentType(RESP_TYPE);
		...
		// Registro de acceso a la API
		restApiService.logAccess(apiKey, HTTP_URL, Map.of("apiKey", apiKey.getKeyToken(), "http-method", "get", "params", params.toString()));
	...
	public String getPlayerById(@RequestHeader(name = API_KEY_HEADER) String apiKeyStr, @PathVariable Long id,
								HttpServletResponse response) throws JsonProcessingException {
	...
	public String partialUpdatePlayer(@RequestHeader(name = API_KEY_HEADER) String apiKeyStr,
		...
		if (player.isEmpty())
			return apiUtils.responseToError(response, HttpServletResponse.SC_NOT_FOUND, DESC_PLAYER_NOT_FOUND);
		...
		if (apiKey.getPlayer().getId().equals(id) || apiKey.getPlayer().getRoles().contains(new Role("ROLE_ADMIN"))) {
			...
			if (playerDto.getUsername() != null) {
				...
				if (err.hasErrors())
					return apiUtils.responseToError(response, HttpServletResponse.SC_BAD_REQUEST, "Invalid username data provided. " + err.getFieldError("username"));
			}
			...
			if (playerDto.getEmail() != null) {
				...
				if (err.hasErrors())
					return apiUtils.responseToError(response, HttpServletResponse.SC_BAD_REQUEST, "Invalid email data provided" + err.getFieldError("email"));
			}
			...
			// Registro de acceso a la API
			restApiService.logAccess(apiKey, HTTP_URL, Map.of("apiKey", apiKey.getKeyToken(), "http-method", "put", "id", id.toString(), "user", playerDto.toString()));
		...
		return apiUtils.responseToError(response, HttpServletResponse.SC_UNAUTHORIZED, "Unauthorized action");
	}

	// Actualizar completamente un jugador (PUT)
	@PutMapping(HTTP_URL + "/{id}")
	@Operation(
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Success",
					content = {@Content(mediaType = RESP_TYPE, examples = {@ExampleObject(name = DESC_EXP,
							value = RESP_SUCCESS)}
					)}),
			@ApiResponse(responseCode = "401", description = DESC_INVALID_KEY,
					content = @Content(mediaType = RESP_TYPE, examples = {@ExampleObject(name = DESC_ERR,
	...
	public String fullUpdatePlayer(@RequestHeader(name = API_KEY_HEADER) String apiKeyStr,
								   @PathVariable Long id, @RequestBody PlayerDto playerDto,
								   HttpServletResponse response) throws JsonProcessingException {
		//Tipo de respuesta
		response.setContentType(RESP_TYPE);
	...
	@ApiResponses(value = {
			...
			@ApiResponse(responseCode = "401", description = "Invalid or Unauthorized api key ",
					content = @Content(mediaType = RESP_TYPE, examples = {
							@ExampleObject(name = DESC_ERR, value = RESP_INVALID_API_KEY),
	...
	public String deletePlayer(@RequestHeader(name = API_KEY_HEADER) String apiKeyStr, @PathVariable Long id,
							   HttpServletResponse response) throws JsonProcessingException {
		//Tipo de respuesta
		response.setContentType(RESP_TYPE);
		...
		if (apiKey.getPlayer().getId().equals(id) || apiKey.getPlayer().getRoles().contains(new Role("ROLE_ADMIN"))) {
			...
			// Registro de acceso a la API
			restApiService.logAccess(apiKey, HTTP_URL, Map.of("apiKey", apiKey.getKeyToken(), "http-method", "delete", "id", id.toString()));
