...
public interface QuestionImageRepository extends CrudRepository<QuestionImage, Long> {
	...
	Page<QuestionImage> findByCategoryAndLanguage(Pageable pageable, Category category,
												  String lang);

	@Query("SELECT q FROM QuestionImage q WHERE LOWER(q.statement) LIKE LOWER(CONCAT('%', ?1, '%')) AND q.language = ?2")
	Page<QuestionImage> findByStatementAndLanguage(Pageable pageable, String statement,
												   String language);
}
