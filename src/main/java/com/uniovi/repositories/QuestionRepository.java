...
public interface QuestionRepository extends CrudRepository<Question, Long> {
	...
	Page<Question> findByCategoryAndLanguage(Pageable pageable, Category category,
											 String lang);

	@Query("SELECT q FROM Question q WHERE LOWER(q.statement) LIKE LOWER(CONCAT('%', ?1, '%')) AND q.language = ?2")
	Page<Question> findByStatementAndLanguage(Pageable pageable, String statement,
											  String language);
}
