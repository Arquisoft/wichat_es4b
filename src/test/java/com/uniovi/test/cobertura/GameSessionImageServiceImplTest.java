...
class GameSessionImageServiceImplTest {
	...
	void getGameSessionsByPlayer_ReturnsPlayerSessions() {
		...
		// Act
		List<GameSessionImage> result = gameSessionService.getGameSessionsByPlayer(
				player);
	...
	void getPlayerRanking_ReturnsPageOfSessions() {
		...
		// Act
		Page<GameSessionImage> result = gameSessionService.getPlayerRanking(pageable,
																			player);
	...
	void startNewGame_ReturnsNewGameSession() {
		// Arrange
		Player player = new Player();
		List<QuestionImage> questions = Arrays.asList(new QuestionImage(),
													  new QuestionImage(),
													  new QuestionImage(),
													  new QuestionImage());
		when(questionService.getRandomQuestions(
				GameSessionImageServiceImpl.NORMAL_GAME_QUESTION_NUM)).thenReturn(
				questions);
	...
	void startNewMultiplayerGame_WithValidCode_ReturnsSession() {
		...
		// Act
		GameSessionImage session = gameSessionService.startNewMultiplayerGame(player,
																			  1234);
	...
	void startNewMultiplayerGame_WithInvalidCode_ReturnsNull() {
		...
		// Act
		GameSessionImage session = gameSessionService.startNewMultiplayerGame(
				new Player(), 9999);
