...
class PlayerServiceImplTest {
	...
	void addNewPlayer_Success() {
		// Arrange
		PlayerDto dto = new PlayerDto("username", "email@test.com", "password",
									  "password", null);
	...
	void addNewPlayer_EmailAlreadyInUse_ThrowsException() {
		...
		assertThrows(IllegalArgumentException.class,
					 () -> playerService.addNewPlayer(dto));
	}
	...
	void updatePlayer_UpdatesFields() {
		// Arrange
		PlayerDto dto = new PlayerDto("newUsername", "newemail@test.com", "newPassword",
									  "password", new String[]{"ROLE_USER"});
		Player player = new Player();
		when(playerRepository.findById(1L)).thenReturn(Optional.of(player));
		Role role = new Role("ROLE_USER");
		when(roleService.getRole("ROLE_USER")).thenReturn(role);
